...
export class AsigDocenteComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  getCollection() {
    this.http
      .get<any>('https://rest-api-parcial.herokuapp.com/maestros').subscribe((res: any) => {
        this.collection = res;
      }, error => {
        console.log({ error });
      })
      this.http
      .get<any>('https://rest-api-parcial.herokuapp.com/curso').subscribe((res: any) => {
  ...
  onCrearAsignacion() {
    if(this.editing){
      ...
      this._router.navigate(['/asigDocentes']);
